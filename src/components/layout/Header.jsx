...
const MOCK_NOTIFICATIONS = [
  { icon: Package, title: "Giao hàng thành công", description: "Đơn #SSE88C1 đã được giao.", time: "5 phút trước", isRead: false },
  { icon: MessageSquareWarning, title: "Cập nhật chính sách", description: "Thay đổi thời gian giao tại Q9.", time: "2 giờ trước", isRead: false },
];
...
export default function Header() {
  ...
  const navigationLinks = NAV_LINKS.reduce((acc, link) => {
    acc.push(link);
    // Only add authenticated links if hydration is complete to avoid mismatch
    if (hasHydrated && isAuthenticated && link.href === "/tracking") {
      acc.push({ ...ORDER_LINK, label: ORDER_LINK.labelAuthenticated });
    }
    return acc;
  }, []);
        ...
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3">
            <Link href="/" className="flex items-center gap-3">
              <Image src="/logo-app.png" alt="Saigon Speed" width={50} height={50} priority className="sm:w-16 sm:h-16 lg:w-12 lg:h-12" />
            </Link>
            <div className="flex items-center gap-2 rounded-full bg-brand-blue/10 px-2 py-1 text-xs font-semibold text-brand-blue">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {hasHydrated ? (
              ...
              <div className="hidden h-10 w-24 animate-pulse rounded-full bg-slate-200 sm:block" />
            )}
                ...
                {hasHydrated && isAuthenticated ? (
                  <>
                    <Link
                      href="/profile"
                      onClick={() => setIsMenuOpen(false)}
                      className="flex items-start gap-3 rounded-lg px-4 py-2 text-base font-medium text-slate-700 hover:bg-slate-100"
                    >
                      <Image src={user.avatar} alt={user.name} width={32} height={32} className="rounded-full mt-1" />
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="popup-arrow absolute right-0 top-full z-40 mt-3 w-64 origin-top-right rounded-2xl bg-white shadow-2xl shadow-slate-500/20 ring-1 ring-slate-900/5"
          >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, scale: 0.95, y: -10 }}
            transition={{ duration: 0.15 }}
            className="popup-arrow hidden sm:block absolute right-0 top-full mt-3 w-96 max-w-sm rounded-2xl bg-white shadow-2xl shadow-slate-500/20 ring-1 ring-slate-900/5 z-50"
          >
            <div className="p-4">
